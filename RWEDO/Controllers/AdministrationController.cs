    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> EditRole(string id)
        {
            ...
            var model = new EditRoleViewModel
            {
                Id = role.Id,
                RoleName = role.Name,
                Claims= roleClaims.Where(c=>c.Value=="true").Select(c => c.Type).ToList(),
            };
        ...
        {
            var users = userManager.Users.Where(x => x.UserName.ToLower() != "sadmin");
            return View(users);
        }
        ...
        public async Task<IActionResult> DeactivateUser(EditUserViewModel model)
        {
            var user = await userManager.FindByIdAsync(model.Id);
            user.IsActive = false;
            var result = await userManager.UpdateAsync(user);
